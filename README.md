...
1. #### Install 
    ...
    Once the runner is setup it will start accepting pending jobs in queue. First it will install all requrest software packeges to EC2 instance (docker, trivy, java, aws cli, kubectl, terraform). 
...
> [!NOTE]
> Don't forget to update the image tag/name in all the places `.github/workflows/cicd.yml` and `manifessts/deployment-service.yml`
...
3. **Terminate Instance**
    ...
    Go to your AWS Management console --> EC2 terminate the created instance (git-workflow) and then remove any additional resources (vpc, security groups, etc)
